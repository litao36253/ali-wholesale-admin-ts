  ...
  .container-wrap-aside
    .box-body
      .menu
        .menu-title {{ $t('系统菜单') }}
        .menu-con
          njs-tree(:data='data' node-key='MENU_ID' ref="tree" default-expand-all :expand-on-click-node='false' check-on-click-node highlight-current @current-change="handleCurrentChange" show-search search-field="MENU_NAME" :placeholder="$t('输入菜单名称搜索菜单')")
            span.custom-tree-node(slot-scope='{ node, data }' v-if="data.showState === 'show'" @mouseenter="mouseIn = data.MENU_ID" @mouseleave="mouseIn = ''")
              span {{ data.MENU_NAME }}
              div(v-show="mouseIn === data.MENU_ID")
                njs-button(type='text', icon='el-icon-plus', @click.stop='append(node, data)')
                njs-button(type='text', icon="el-icon-edit" :disabled="data.IS_DEL === '2' || data.IS_DEL === '3'" @click="data.showState = 'edit'" v-if="data.MENU_ID !== '0'")
                njs-button(type='text', icon="el-icon-delete" :disabled="data.IS_DEL === '1' || data.IS_DEL === '3'" @click='delNode(node, data)' v-if="data.MENU_ID !== '0'")
            span.custom-tree-node(slot-scope='{ node, data }' v-else)
              njs-form(:model="data" :ref="`form${data.MENU_ID}`" :rules="addEditRules" :style="{flex:1, marginRight: '4px'}" size="mini" @submit.native.prevent @click.stop)
                njs-form-item(prop="editName" :style="{marginBottom: '0'}")
                  njs-input(v-model="data.editName" :ref="`input${data.MENU_ID}`" @keydown.native.13="doEdit(node, data)")
      ...
      .form-wrap
        ...
        .form
          njs-form(:model="formData" ref="addEditForm" :service="currentNodeData.showState==='add' ? 'P0104101' : 'P0104302'" :rules="addEditRules" @before-submit="handleSubmitHandle" @submit-success="handleSuccessHandle" size="default" v-if="formShow" :disabled="!currentNodeData.MENU_ID || currentNodeData.MENU_ID === '0' || currentNodeData.IS_DEL === '2' || currentNodeData.IS_DEL === '3'")
            njs-form-item(prop="MENU_LINK" :label="$t('菜单路径')")
              njs-input(v-model="formData.MENU_LINK")
            njs-form-item(prop="MENU_ICO" :label="$t('菜单图标')")
              njs-input(v-model="formData.MENU_ICO")
            njs-form-item(prop="IS_DEL" :label="$t('维护标识')")
              njs-radio-group(v-model="formData.IS_DEL" dict="MAINTAIN_FLAG" :disabled="currentNodeData.showState !== 'add' && currentNodeData.IS_DEL !== '0'")
            njs-form-item(prop="MENU_STAT" :label="$t('启用状态')")
              njs-switch(v-model="formData.MENU_STAT" active-color="#13ce66" active-value="1" inactive-value="0" inactive-color="#ff4949" :active-text="$t('启用')" :inactive-text="$t('禁用')")
</template>
...
export default {
          ...
          { validator: (rule, value, callback) => {
            if (value === '') {
              ...
              if (/^(\/[-A-Za-z\d]+)+$/.test(value)) {
                ...
                callback(new Error(this.$t('请输入正确路径，形如：/system/menu')))
              }
            }
          },
          trigger: 'change' },
          { max: 64, message: this.$t('长度不能超过 64'), trigger: 'change' }
        ],
        MENU_STAT: [],
        MENU_ICO: []
      },
  ...
  methods: {
      ...
      if (data.showState === 'add') { // 新增
        this.$refs.addEditForm.validate((result) => {
          if (result) {
            ...
            this.$message.error(this.$t('请先完善菜单信息'))
          }
        })
      } else {
        this.$ajaxRequest([{
        ...
        this.$refs[`form${this.currentNodeData.MENU_ID}`].validate((result) => {
          if (result) {
            ...
            if (this.currentNodeData.editName) {
              this.$message.error(this.$t('菜单名称长度不能超过 64！'))
            } else {
              this.$message.error(this.$t('请输入菜单名称！'))
            }
            back(false)
          }
        })
    ...
    // 删除节点
    delNode (node, data) {
      this.$confirm(this.$t('你确定要删除') + data.MENU_NAME + this.$t('菜单及其所有子菜单吗？'), this.$t('提示'), {
        type: 'warning'
      }).then(() => {
        this.$ajaxRequest([{
          MENU_ID: data.MENU_ID,
          ...
          if (result.code === '0') {
            ...
            this.$message({
              type: 'success',
              message: this.$t('删除') + data.MENU_NAME + this.$t('菜单及其所有子菜单成功')
            })
          }
        })
      }).catch(() => {
  ...
  created () {
    this.$ajaxRequest([{
      ...
      if (result.code === '0') {
        ...
        this.$set(this.data[0], 'children', listTransTree(result.data, 'MENU_ID', 'PAR_MENU'))
      }
    })
  },
  computed: {
    formTitle () {
      if (this.currentNodeData.MENU_ID) {
        if (this.currentNodeData.MENU_ID === '0') {
          return "<span style='color: #ff4949'>" + this.$t('根菜单不可编辑') + '</span>'
        }
        return this.currentNodeData.editName || ("<span style='color: #ff4949'>" + this.$t('未输入菜单名称') + '</span>')
      } else {
        return "<span style='color: #ff4949'>" + this.$t('未选中菜单') + '</span>'
      }
    }
  }
}

    ...
    <div class="wscn-http404">
      ...
      <div class="bullshit">
        ...
        <div class="bullshit__info">{{ $t('版权所有') }}
          <a class='link-type' href='http://www.netinfosz.com/' target='_blank'> NETINFO </a>
        </div>
        <div class="bullshit__headline">{{ message }}</div>
        <div class="bullshit__info">{{ $t('请检查您输入的网址是否正确，点击以下按钮可返回主页') }}</div>
        <span @click="$router.push('/dashboard')" class="bullshit__return-home">{{ $t('返回首页') }}</span>
      </div>
    </div>
  </div>
</template>
